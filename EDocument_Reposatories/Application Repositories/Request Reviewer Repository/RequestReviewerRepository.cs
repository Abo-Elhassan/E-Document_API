    ...
    public class RequestReviewerRepository : GenericRepository<RequestReviewer>, IRequestReviewerRepository
    {
        ...
        public async Task BeginRequestCycle(long definedRequestId, long requestId, string requesterId, bool isNew)
        {
            ...
            if (firstReviewer != null)
            {
                ...
                if (request.RequestReviewers.Any(r => r.AssignedReviewerId == request.CreatorId) && definedRequestId!= 2023110902543453 && definedRequestId!= 2023110902550379 && definedRequestId != 2023101010522512) //Automatic approve for the creator should be prevented for Refund and  New Item and Pr Requests as store team need to insert data in approve
                {
